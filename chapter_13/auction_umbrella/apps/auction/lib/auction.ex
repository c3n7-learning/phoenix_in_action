defmodule Auction do
  @moduledoc """
  Provides functions for interacting with the databse layer of an Auction
  application

  In order to keep database concerns separate from the rest of an application,
  these functions are provided. Any interaction you need to do with the
  database can be done from withing these function. See an individual function's
  documentation for more information and usage examples (like
  [Auction.get_user_by_username_and_password/2](#get_user_by_username_and_password/2)).
  """
  ...
  ## Return values
  Depending on what is found in the database, two different values cold be returned:
  ...
  * `false`: No `Auction.User` could be found with the provided `username`
  ...
  authorized in your application. In an `Auction.User` is _not_ found based on
